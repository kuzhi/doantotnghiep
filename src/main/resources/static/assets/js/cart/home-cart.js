...
app.controller("cart-ctrl", function($scope, $http, $location) {
	...
	$scope.countAmount = function(storeid) {
		if (storeid != 0 && $scope.userid != 0) {
			$http
				.get("/api/countcart/" + storeid + "/" + $scope.userid)
				.then((resp) => {
		...
		$http.get($scope.url + "/store/" + $scope.sid + "/" + true).then((resp) => {
			...
			if ($scope.products.length == 0) {
				...
				swalWithBootstrapButtons.fire({
					title: 'Thông báo',
					icon: 'warning',
					text: "Cửa hàng chưa có sản phẩm!",
					...
					if (result.isConfirmed) {
						...
						//====================================== Bắt đầu xử lý
						document.getElementById("listStore").click()
						//====================================== Kết thúc xử lý
					} else if (
	...
	//   $scope.getUsersbyFilter = function (item) {
	//     // var index = $scope.selected.indexOf(item);
	//     // if (index > -1) {
	//     //   $scope.selected.splice(index, 1);
	//     // } else {
	//       $http.get("/api/product/" + "sort/" + item).then((resp) => {
	//         $scope.products = resp.data;
	...
	$scope.totalMoney = function() {
		// lệnh này thì dùng để tính tổng số tiền các mặt hàng trong giỏ
		return $scope.items
	...
	$scope.deleteall = function() {
		//xóa hết sp trong giỏ
		$http
			.delete("/api/cart/deleteall/" + $scope.sid + "/" + $scope.userid)
			.then((resp) => { });
		$scope.loadCart($scope.sid, $scope.userid);
	};
	...
	$scope.order = {
		...
		get orderDetail() {
			return $scope.items.map((item) => {
				//lệnh này thì lấy cái đống dữ liệu trong mảng item để mình tạo ra cái list chi tiết đơn hàng
				return {
	...
	$scope.checkaddress = function(s) {
		if (s.id == 1) {
			$scope.order.address = $scope.myProfile.address;
		} else if ((s.id = 2)) {
			$scope.order.address = $scope.store.address;
		}
	};
		...
		swalWithBootstrapButtons
			...
			.then((result) => {
				if (result.isConfirmed) {
					...
					var getSelectedValuePay = document.querySelector(
						'input[name="payment"]:checked'
					);
					var getSelectedValueShip = document.querySelector(
						'input[name="shipping"]:checked'
					);

					if ($scope.order.fullname === "") {
						$scope.order.fullname = $scope.myProfile.fullname;
					}
					if ($scope.order.fullname.length < 4) {
						swalWithBootstrapButtons.fire(
							"Thông báo",
							"Tên của bạn phải từ 3 ký tự!",
							"warning"
						);
					} else if ($scope.order.phone === "") {
						swalWithBootstrapButtons.fire(
							"Thông báo",
							"Vui lòng cung cấp số điện thoại nhận hàng!",
							"warning"
						);
						...
						swalWithBootstrapButtons.fire(
							"Thông báo",
							"Vui lòng cung cấp số địa chỉ nhận hàng!",
							"warning"
						);
					} else if ($scope.order.address.length < 4) {
						swalWithBootstrapButtons.fire(
							"Thông báo",
							"Địa chỉ của bạn phải từ 3 ký tự!",
							"warning"
						);
		...
		swalWithBootstrapButtons
			...
			.then((result) => {
				if (result.isConfirmed) {
					...
					$http
						.get($scope.urlInfo + "email/" + user.email + "/" + user.id)
						.then((resp) => {
							const myEmail = resp.data;
							// tìm ra thì tìm sdt
							if (myEmail.length != 0) {
								$http
									.get($scope.urlInfo + "phone/" + user.phone + "/" + user.id)
									.then((resp) => {
										...
										if (myPhone.length != 0) {
											...
											// tìm k ra thì check trùng
											// Check phone
											$http
												.get($scope.urlInfo + "phone/" + user.phone)
												.then((resp) => {
													...
													if (checkPhone.length != 0) {
														...
														// neu phone moi k trung thi update
														$scope.updateReal(user);
													}
												})
												.catch((error) => {
													Swal.fire({ icon: "error", title: "Lỗi!" + error });
													console.log(error);
												});
										}
									})
								...
								// Check email
								$http
									.get($scope.urlInfo + "email/" + user.email)
									.then((resp) => {
										const checkEmail = resp.data;
										if (checkEmail.length != 0) {
											Swal.fire({
												icon: "warning",
												title: 'Email "' + user.email + '" đã tồn tại!',
											});
										} else {
											$http
												.get(
													$scope.urlInfo + "phone/" + user.phone + "/" + user.id
												)
												.then((resp) => {
													const myPhone = resp.data;
													// tìm ra thì cập nhật
	...
	$scope.updateReal = function(user) {
		$http
			.put($scope.urlInfo + "updateProfile/" + user.id, user)
			.then((resp) => {
	...
	$scope.loading = function() {
		...
		$http
			.get("/api/order/" + $scope.sid + "/" + $scope.userid + "/" + 1)
			.then((resp) => {
				$scope.orders = resp.data;
			});
	};
	...
	$scope.shipping = function() {
		...
		$http
			.get("/api/order/" + $scope.sid + "/" + $scope.userid + "/" + 2)
			.then((resp) => {
				$scope.orders = resp.data;
			});
	};
	...
	$scope.completed = function() {
		...
		$http
			.get("/api/order/" + $scope.sid + "/" + $scope.userid + "/" + 3)
			.then((resp) => {
				$scope.orders = resp.data;
			});
	};
	...
	$scope.canceled = function() {
		...
		$http
			.get("/api/order/" + $scope.sid + "/" + $scope.userid + "/" + 4)
			.then((resp) => {
				$scope.orders = resp.data;
			});
	};
